...
async function getUpstreamLatest() {
  ...
  if (!commit?.sha || !commit?.commit?.committer?.date || !commit?.commit?.message) {
...
async function main() {
  ...
  try {
    ...
    // Manual sync required — do not auto-pull to avoid supply-chain risk
    console.log(`\n       To update manually:`)
    console.log(`         cd skills/ui-ux-pro-max`)
    if (hasGitRemote()) {
      console.log(`         git pull origin main --ff-only`)
    } else {
