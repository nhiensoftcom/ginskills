...
TARGET="${1:-all}"
case "$TARGET" in
  all|backend|frontend|mobile|supply-chain|llm|secrets) ;;
  *) echo "  [error] Invalid target: $TARGET"; echo "  Valid targets: all, backend, frontend, mobile, supply-chain, llm, secrets"; exit 1 ;;
esac
...
finding() {
  ...
  echo "         → $file"
  FINDINGS+=("{\"severity\":\"$sev\",\"category\":\"$category\",\"file\":\"$file\",\"detail\":$(printf '%s' "$detail" | python3 -c 'import json,sys; print(json.dumps(sys.stdin.read()))' 2>/dev/null || echo "\"$detail\"")}")
  case $sev in
...
scan_supply_chain() {
  ...
  if [ -f "$dir/package.json" ]; then
    while IFS= read -r dep_dir; do
      ...
      if grep -q '"preinstall"\|"postinstall"' "$pkg_json" 2>/dev/null; then
        local pkg_name
        pkg_name=$(python3 -c 'import json,sys; print(json.load(sys.stdin).get("name","unknown"))' < "$pkg_json" 2>/dev/null || echo "unknown")
  ...
  if [ -f "$dir/node_modules/@nestjs/devtools-integration/package.json" ]; then
    local devtools_ver
    devtools_ver=$(python3 -c 'import json,sys; print(json.load(sys.stdin).get("version","0.0.0"))' < "$dir/node_modules/@nestjs/devtools-integration/package.json" 2>/dev/null || echo "0.0.0")
